...
class SecAggPlusWorkflowWithLogs(SecAggPlusWorkflow):
    ...
    def __call__(self, driver: Driver, context: Context) -> None:
        ...
        log(
            INFO,
            "SecAgg+ performs a secure sum of clients contributions.",
        )
        log(
            INFO,
        ...
        log(
            INFO,
            "The raw vectors are hidden from the ServerApp through adding masks.",
        )
        log(INFO, "")
        log(
            INFO,
        ...
        log(
            INFO,
            "Result of secure sum (dequantized): %s",
            ndarrays[0].flatten()[:10],
        )
        log(INFO, "")
        log(
            INFO,
